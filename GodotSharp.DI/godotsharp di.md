...
partial class MyServiceScope
{
    ...
    private void RegisterServiceInstances()
    {
        ...
        // 注册为 SingletonService 特性指定的类型
        // 如果未指定则注册为原类型
        RegisterInstance<IDataWriter>(databaseWriter);
        RegisterInstance<IDataReader>(databaseWriter);
    }
...
partial class MyServiceScope
{
    ...
    private static object? GetTransient(Type type)
    {
        // 在此创建 TransientService 瞬态服务
        // 注册为 TransientService 特性指定的类型
        if (type == typeof(PathFinder))
        {
    ...
    {
        var type = typeof(T);
        var transient = GetTransient(type);
        if (transient is not null)
        {
            onResolved.Invoke((T)transient);
            return;
        }
